# Домашние задания по курсу «Основы Java»

[Глоссарий терминов по основам Java] (https://github.com/netology-code/java-homeworks/blob/master/glossary_basic_java)

### Блок 1. Основы Java

[1.1	Первая программа на Java](./introduction/)

[1.2	Структура программы](./program-structure/)

[1.3. Условные операторы и циклы](./conditional-statements-cycles/)

[1.4	Типы данных в Java: примитивы](./primitive-types)

[1.5	Типы данных в Java: объекты](./reference-types)


### Блок 2. InMemory хранение данных — массивы

[2.1	Массивы одномерные](/arrays/3.1/)

[2.2	Массивы многомерные](/arrays/3.2/)


### Блок 3. Основы ООП

[3.1	Структура класса](./class-structure)

[3.2	Модификаторы доступа, наследование](./inheritance)	

[3.3	Полиморфизм](./polymorphism/)

[3.4	Абстракции и интерфейсы](./abstractions-interfaces/)

[3.5 Исключения, Stacktrace](./exceptions)



### Блок 4. InMemory хранение данных — коллекции

[4.1	Коллекции List](/in-memory_list/)

[4.2	Коллекции Queue](/in-memory-queue/)

[4.3	Коллекция HashSet и HashMap](/hash-collections/)

[4.4	Коллекции TreeSet](/in-memory-treeset/)

[4.5	Коллекции TreeMap](/in-memory-treemap/)
