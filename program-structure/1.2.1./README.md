...
4. Вывести на экарн сумму этих чисел

### Пример
```
Программа рассчета суммы трех чисел
...
```
7. Создадим метод принимающий 3 целых числа в качестве аргументов, назовем его `sum`,
он будет складывать и возвращать сумму трех чисел.
```java
static int sum(int value1, int value2, int value3) {
    return value1 + value2 + value3;
}
```
8. В методе `public static void main` запросим у пользователя ввести три числа, которые в дальнейшем передадим
в качестве аргументов методу `sum`
```java
Scanner scanner = new Scanner(System.in);
