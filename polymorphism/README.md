# Домашнее задание к лекции 2.3 Основы ООП: полиморфизм
...
> ВНИМАНИЕ
> Все программисты **считают начиная с нуля**! Первый элемент списка можно получить с помощью `list.get(0)`, второй `list.get(1)` соответсвенно.
...
Один из проектов это игра-шутер (~~Half-Life 3, только никому ни слова~~). У игрока должна быть возможность использовать разные виды оружия, в будущем в игру могут быть добавлены новые. Необходимо спроектировать иерархию классов, а так же систему слотов для оружия у игрока.
...
Как и `String` экземпляры этого класса неизменяемые (иммутабельны), а методы `.add(…)`, `.multiply(…)` возвращают новый объект содержащий резульат операции.
