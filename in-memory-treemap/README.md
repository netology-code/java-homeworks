...
4. Так как выводить жанры требутеся по условиям задачи так же в упорядоченном виде, создадим TreeMap, где будем
...
5. Нужно создать отдельный класс, назовем его `BookCatalog.java` для работы с книгами, он должен реализовывать методы:
  - добавлние книги и жанра `public void addBook(Book book)`.
