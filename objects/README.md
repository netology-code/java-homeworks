...
1. Для начала нужно считать данные для разбора.
  ...
  - Для того чтобы прочитать значения нужно вызвать у объекта `scanner.next()`, но так этот метод считвает каждое новое значение через пробел лучше использовать другой метод этого класса - `scanner.nextLine()`, он считает всю строку целиком. Сохраним его в переменную, которую назовем `input`
    ```
