...
Первым делом [скачайте проект задания](https://github.com/netology-code/java-homeworks/raw/master/diploma/javabasicsdiploma.zip), разархивируйте архив с проектом и откройте его в IntelliJ IDEA (File -> Open -> выберите скачанную папку). В папке assets менять ничего нельзя, там лежит реализация страницы приложения. В папке src лежит исходный код приложения. Класс в пакете server трогать нельзя - он написан за вас, он будет использовать объект интерфейса, который вам нужно реализовать.
